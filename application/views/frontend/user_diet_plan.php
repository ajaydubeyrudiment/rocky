  ...
  <form id="workoutItems" method="post" onsubmit="return false;">
    <div class="feed_right">
      <div class="goal_setter">
        <h2>Diet Plan - <?php echo !empty($currentPlan->plan_name)?ucfirst($currentPlan->plan_name):''; ?><span class="pull-right metrics_date">
          <?php
            $wday          = date('w');
        ...
        <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
          <?php 
            ...
            if(!empty($days)){
              $i=1;
              foreach($days as $day){                
                $dayDetails = $this->developer_model->getDietPlanDay($day, $this->input->get('plan_id'), $this->input->get('user_id'));                
                ?>
          ...
          <div class="followsss text-center">
            <a class="following-settingss" href="javascript:void();" onclick="copy_diet_plan('<?php if($this->input->get('plan_id')){ echo $this->input->get('plan_id');} ?>','<?php if($this->input->get('goal_id')){ echo $this->input->get('goal_id');} ?>','<?php if($this->input->get('user_id')){ echo $this->input->get('user_id');} ?>');">
              Copy
            </a>
          </div>
