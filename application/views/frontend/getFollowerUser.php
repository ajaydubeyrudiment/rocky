...
if(!empty($rows)){
	foreach($rows as $row){		
		...
		<div class="media" >
		    ...
		    <div class="media-body">
		      	<?php
		      	//echo get_all_count('follow_request', array('sender_id' => user_id(), 'receiver_id' => $row->user_id, 'accepted_status'=>1)); 
		      	if($row->user_id==user_id()){?>
		    	...
		    	<?php }?>
		      	<h4>
		      		<a href="<?php echo $userNameIDS ?>">
		      			<?php echo !empty($row->user_name)?ucfirst($row->user_name):'' ?>
		      		</a>
		      	</h4>
		    </div>
		</div>
	...
	//echo '<h3 class="text-center text-danger"> No follower records found</h3>';
}
?>