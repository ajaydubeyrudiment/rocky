...
<section class="content-header tab_header">
  ...
  <ol class="breadcrumb">
    ...
    <li class="active">Diet Item List</li>
  </ol>
</section>
<section class="content-header" id="filter_main">
  <form action="" method="get">
    <div class="filter_box" style="width:150px">
      <label>Food ID</label>
      <input type="text" name="food_id" value="<?php if($this->input->get('food_id')){echo $this->input->get('food_id');}?>" class="form-control"  placeholder="Search Food ID">
    </div>
    <div class="filter_box" style="width:">
      <label>Food</label>
      <input type="text" name="food" value="<?php if($this->input->get('food')){echo $this->input->get('food');}?>" class="form-control"  placeholder="Search Food">
              ...
              foreach ($rows as $res_data){?>
                <tr>
                  ...
                  <td class="text-center">
                    <?php 
                    if(!empty($res_data->exercise_pic)&&file_exists('assets/uploads/planItem/thumbnails/'.$res_data->exercise_pic)){
                      echo '<img src="'.base_url().'assets/uploads/planItem/thumbnails/'.$res_data->exercise_pic.'" width="50"/>';
                    }else if(!empty($res_data->exercise_pic)&&file_exists('assets/uploads/plansExercise/thumbnails/'.$res_data->exercise_pic)){
                      echo '<img src="'.base_url().'assets/uploads/plansExercise/thumbnails/'.$res_data->exercise_pic.'" width="50"/>';
                    }else{
                      echo '-';
                    }
                    ?>
                  ...
                  <td class="text-left">
                    <?php  echo !empty($res_data->item_title)? ucfirst($res_data->item_title):'-';?>
                  </td>
                  <td class="text-left">
                    ...
                    <div class="dropdown">
                      ...
                      <ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
                        ...
                        <li role="presentation">
                          <a role="menuitem" tabindex="-1" href="javascript:void(0);" onclick="changeStatus('service_plan_diet_items','diet item','<?php if(!empty($res_data->id)) echo $res_data->id; ?>','<?php echo $k; ?>','<?php if($k==1){ echo 'activate';}else if($k==3){ echo 'delete';}else{ echo 'deactive';}?>','id','status');">
                            <?php echo $status_a; ?>
                          </a>
                        </li>
                        <?php } 
                  ...
                  <td class="text-center"> 
                    <a class="btn btn-sm btn-primary" href="<?php echo ADMIN_URL.'servicePlan/add_diet_plan_food/';?><?php if(!empty($res_data->id)) echo $res_data->id;?>">
                      <i class="fa fa-pencil" aria-hidden="true"></i>
                   </a>   
                  <a class="btn btn-sm btn-danger" href="javascript:void(0);" onclick="changeStatus('service_plan_diet_items','diet item','<?php if(!empty($res_data->id)) echo $res_data->id; ?>','3','delete','id','status');");">
                  ...
                  <tr >
                    <td colspan="8" class="text-center text-danger">
                      <span class="data-not-present">
                        <?php echo 'No diet items records found'; ?>                
                      </span>
                    </td>
                  </tr>              
              <?php } ?> 
